...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='MusicBot, REEEEEEEE', icon_url='https://i.imgur.com/gFHBoZA.png')
    ...
    async def cmd_play(self, message, player, channel, author, permissions, leftover_args, song_url):
        ...
        if self.is_baned(str(song_url)):
            return Response("Bot says, NO!", delete_after=30)
    ...
    async def cmd_forceplay(self, player, channel, author, permissions, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            try:
                entry, position = await player.playlist.add_entry_front(song_url, channel=channel, author=author)
                player.skip()
            except exceptions.WrongEntryTypeError as e:
                if e.use_url == song_url:
        ...
        return Response("Will do", delete_after=30)
